...
int RunCLI(int argc, char **argv)
{
    ...
    bool quiet = false;
    app.add_flag("-q,--quiet", quiet, "只输出错误与最终统计，抑制逐文件进度");
    ...
    app.footer("示例：\n"
               "  osp -i song.osu -o out -s 1.5\n"
               "  osp -i songs/ -r -o out -t 3/2\n"
               "  osp -i song.osu -o out -d\n"
    ...
    g_useColor = forceColor || DetectColor();
    ...
    {
        mapping = LoadDiffMapping(diffFile);
        if (mapping->empty())
        {
            std::cerr << Yellow("[警告] ") << "映射文件为空：" << diffFile << std::endl;
        }
    }
    ...
    if (!quiet)
    {
        ...
        if (verbose)
        {
            std::cout << Dim("输出目录：" + core::fs::absolute(output).string()) << std::endl;
        }
        std::cout << std::endl;
    }
